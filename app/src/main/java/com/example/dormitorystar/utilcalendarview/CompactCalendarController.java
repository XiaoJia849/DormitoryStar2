package com.example.dormitorystar.utilcalendarview;

import static com.example.dormitorystar.utilcalendarview.CompactCalendarView.FILL_LARGE_INDICATOR;
import static com.example.dormitorystar.utilcalendarview.CompactCalendarView.NO_FILL_LARGE_INDICATOR;
import static com.example.dormitorystar.utilcalendarview.CompactCalendarView.SMALL_INDICATOR;
...
import com.example.dormitorystar.R;
import com.example.dormitorystar.utilcalendarview.CompactCalendarView.CompactCalendarViewListener;
            ...
            for (int i = 0; i < uniqEvents.size(); i++) {
                ...
                if (shouldDrawIndicatorsBelowSelectedDays || (!shouldDrawIndicatorsBelowSelectedDays && !isSameDayAsCurrentDay && !isCurrentSelectedDay) || animationStatus == EXPOSE_CALENDAR_ANIMATION) {
                    if (eventIndicatorStyle == FILL_LARGE_INDICATOR || eventIndicatorStyle == NO_FILL_LARGE_INDICATOR) {
                        ...
                        //    在数字周围画圈
                        drawNum(canvas,xPosition,yPosition-indicatorOffset/2,eventsList);
...
//    中心不对
    ...
    private static float zeroCircleRadius=40f;
    private static float threeRadius=35f;
    ...
    private void drawFour(Canvas canvas,float xPosition,float yPosition){
        canvas.drawRect(xPosition-fourRadius,yPosition+fourRadius,xPosition+fourRadius,
                    yPosition-fourRadius,dayPaint);
    }

    private void drawFive(Canvas canvas,float xPosition,float yPosition){
        Path path=new Path();
        float[] floats= fivePoints(xPosition,yPosition-fiveRadius_big/2,fiveRadius_big);
        path.moveTo(floats[0], floats[1]);
        for (int i = 2; i < floats.length - 1; i++) {
    ...
    private void drawNum(Canvas canvas, float xPosition, float yPosition, List<Event> eventsList) {
...
//                dayPaint.setColor(Color.argb(255, 169, 68, 65));
     ...
     * @param rFive 五角星边的边长
     */
    public  float[] fivePoints(float xA, float yA, int rFive) {
        float xB = 0;
        float xC = 0;
        float xD = 0;
        float xE = 0;
